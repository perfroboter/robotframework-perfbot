...
def generate_report(opts):
    ...
    logging.info("Database is selected")
    persistenceService: PersistenceService = Sqlite3PersistenceService(opts.perf_db)
    ...
    if opts.showkeyword == "True":
        ...
        # result.visit(KeywordResults(kw_list,None,None))#, #IGNORE_LIBRARIES))
         hide_keyword_menu = ""
    else:
